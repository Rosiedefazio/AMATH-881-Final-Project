...
"""
import os
os.environ["MPLBACKEND"] = "Agg"          # ← alternative to matplotlib.use()
import matplotlib
matplotlib.use("Agg") 
...
plotData = fullmodel.sample('run1', coords=['a', 'ad'], dt=0.01)

afig = plt.figure(figsize=(6, 4))
aline = plot(plotData['t'], plotData['a'],
             color='blue', linewidth=1.5, label=r'$\alpha(t)$')
adline = plot(plotData['t'], plotData['ad'],
             color='orange', linewidth=1.5, label=r'$\dot\alpha(t)$')

plt.xlabel('time  $t$')
plt.ylabel('state')
plt.title('Finger‑model dynamics (VODE)')
plt.legend(loc='best')
plt.grid(True, which='both', ls=':', alpha=0.5)

output_path = "fingermodel_vode.png"
